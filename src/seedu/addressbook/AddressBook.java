...
public class AddressBook {
    ...
    /* We use a HashMap to store details of a single person.
       The keys of different elements are stored in an enumeration.
     */
     ...
     */
    private static void processProgramArgs(String[] args) { //Avoid deep nesting
        if (args.length >= 2) {
    ...
    private static boolean hasValidFileName(Path filePath) {
        boolean hasValidFormat = filePath.getFileName().toString().lastIndexOf('.') > 0;
        boolean isExisting = Files.exists(filePath);
        boolean isRegular = Files.isRegularFile(filePath);
        return hasValidFormat
                && (!isExisting || isRegular);
    }
    ...
    */
    ...
    private static String getUsageInfoForAllCommands() {
        final String usageInfo = getUsageInfoForAddCommand() + LS
